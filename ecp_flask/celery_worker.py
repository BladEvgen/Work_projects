    ...
    try:
        ...
        for file_path in renamed_file_paths:
            try:
                ...
                logger.info(
                    f"File signed successfully: {file_path}, file_id: {file_id}"
                )
                ...
                if verification_info:
                    ...
                    save_to_database(
                        original_filename, signed_file_path, "async_sign_file", "OK"
                    )
                else:
                    logger.error(
                        f"Verification info could not be retrieved for file: {file_path}"
                    )
                    continue
        ...
        for processed_file in processed_files:
            try:
                ...
                logger.exception(
                    f"Error finalizing file {processed_file['original_filename']}: {str(e)}"
                )
        ...
        if package and signed_files:
            ...
            if filenames:
                ...
                with db_connection_certificate.cursor() as cursor:
                    ...
                    db_connection_certificate.commit()
                logger.info(
                    f"Updated signed_status in certificate table for id: {filenames}"
                )
            else:
                logger.warning(
                    "No filenames found in signed_files to update the database."
                )
